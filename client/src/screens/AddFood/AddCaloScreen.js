...
const AddCalo = ({ navigation }) => {
  ...
  const calculateTotalCalories = (dishes) => dishes.reduce((total, dish) => total + dish.Calories, 0);
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          text: 'Xóa',
          onPress: () => {
            // Dispatch action to delete the meal from the user
            dispatch(deleteFoodMealFromUser(userId, dishId))
              .then(() => {
                // After successful deletion, fetch the updated meals data again
